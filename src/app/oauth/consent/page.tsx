...
export default function OAuthConsentPage() {
  return (
    <Suspense fallback={<LoadingSpinner message="Loading authorization request..." />}>
      <OAuthConsentContent />
    </Suspense>
  );
}
...
function OAuthConsentContent() {
  ...
  if (!authorizationId) {
    return (
      <div className="max-w-md mx-auto mt-20 px-4">
        <Alert variant="error">Missing authorization request. Please restart the connection.</Alert>
      </div>
    );
  }
  ...
  if (loading) {
    return <LoadingSpinner message="Loading authorization request..." />;
  }
